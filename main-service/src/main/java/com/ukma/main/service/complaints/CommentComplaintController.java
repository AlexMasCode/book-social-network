...
public class CommentComplaintController {
    ...
    @PostMapping
    public ResponseEntity<?> createCommentComplaint(@RequestBody CommentComplaintDto commentComplaintDto){

        return ResponseEntity.ok(commentComplaintService.createCommentComplaint(commentComplaintDto));
    }

    @PatchMapping("/{id}")
    public ResponseEntity<?> resolveComplaint(@PathVariable("id") Long id, @RequestBody String status){

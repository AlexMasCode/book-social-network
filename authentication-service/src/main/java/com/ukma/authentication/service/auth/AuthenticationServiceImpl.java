...
public class AuthenticationServiceImpl {
    ...
    public TokenDto login(AuthDto authDto) {
        User userCheck = userRepository.findByUsername(authDto.getEmail()).orElse(null);
        if (userCheck != null) {
    ...
    public TokenDto register(AuthDto authDto) {
        Optional<User> userCheck = userRepository.findByUsername(authDto.getEmail());
        if (userCheck.isEmpty()) {
            User newUser = new User(authDto.getEmail(), passwordEncoder.encode(authDto.getPassword()));
